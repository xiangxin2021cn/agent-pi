...
3. Use only the backend-generated `boq_batch_manifest_path`, task board, and child briefs. Each batch contains at most 12 ordered items with immutable BOQ row data and linked scope sources. Do not enlarge, regroup, or bypass a batch.
4. Complete all five steps for every assigned item:
   ...
   - **Step 4 - sourced rates and pure direct cost:** for every component record per-unit quantity, unit, rate, amount, source locator, source type, acquisition mode, location, effective date, VAT-exclusive basis, and verification state. Reconcile category subtotals, unit direct cost, and `itemDirectCost = unitDirectCost x BOQ quantity`.
...
6. Wait until the task board reports every child handoff schema-valid. Never price an unfinished child range in the main session. Merge only child reports and run duplicate ownership, full-item coverage, source, unit, quantity, productivity, resource, rate, arithmetic, and cross-batch conflict checks.
7. Call `tender_capability` with `configure` for `boq_five_step_pricing`.
8. Call `tender_capability` with `init`, or `replace` with the current `expectedRevision`.
9. Call `validate`. Resolve every error before downstream planning. Initial item cash-flow allocations are optional here; schedule-based cash flow belongs to the later cost/cash-flow stage.
