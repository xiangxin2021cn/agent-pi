...
- Child agents write **both** the structured JSON handoff (`reportPath`) and the customer-facing chapter Markdown (`markdownPath`). Session work products go to Official Outputs (`Agent Pi Outputs/<sessionId>/`). Once every batch is accepted, the **runtime** merges JSON into the capability pack; the main session must never hand-assemble, compress, or rewrite pricing content into `tender_capability`.
...
7. After a successful pricing merge, the runtime aggregates `resourceConsumptions × BOQ quantities` into `construction_resource_schedule` and writes `施工资源消耗总表.md` under Official Outputs. Do not hand-assemble that schedule in the main session.
