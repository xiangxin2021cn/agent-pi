...
- Professional document requests for construction schedules, investment analysis, GIS/geospatial reports, and simulation/CAE reports now add a visual-block criterion and a domain visual plan. The audit attaches `visual_block_audit` evidence and fails pure prose when required visuals, captions, source notes, or industry context are missing.
- Strict reference-template requests now add a template-fidelity criterion. The audit attaches `template_fidelity_audit` evidence and rejects prompt-only template claims; strict DOCX work requires a parsed template profile and exported DOCX structure evidence before it can pass.
