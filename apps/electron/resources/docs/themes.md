...
3. **Preset themes**: `~/.agent-pi/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.agent-pi/theme.json` - Override specific colors (app-level)
...
```
~/.agent-pi/workspaces/{id}/config.json
```
...
Create `~/.agent-pi/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.agent-pi/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.agent-pi/themes/{name}.json`
...
1. Create `~/.agent-pi/theme.json` for overrides or `~/.agent-pi/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.agent-pi/theme.json` for overrides, `~/.agent-pi/themes/` for presets)
