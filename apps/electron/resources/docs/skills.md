...
1. **Workspace skill checked first** - If `~/.agent-pi/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.agent-pi/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.agent-pi/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.agent-pi/workspaces/{ws}/skills/commit/SKILL.md`
