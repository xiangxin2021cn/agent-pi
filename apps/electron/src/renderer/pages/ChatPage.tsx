...
const ChatPage = React.memo(function ChatPage({ sessionId }: ChatPageProps) {
  ...
  const handleOpenInNewWindow = React.useCallback(async () => {
    ...
    const url = `agentpi://${route}${separator}window=focused`
    try {
  ...
  const sessionInfoSidebar = !isCompactMode && sessionMeta ? (
    <aside className="hidden xl:flex h-full w-[320px] min-w-[300px] max-w-[360px] shrink-0 border-l border-border/60 bg-background/95">
      <SessionInfoPanel
        sessionId={sessionId}
        sessionFolderPath={session?.sessionFolderPath}
        className="w-full"
