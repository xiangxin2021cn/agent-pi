 ...
 * Deep Link Handler
 *
 * Parses agentpi:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   agentpi://allSessions[/session/{sessionId}]            - Session list (all sessions)
 *   agentpi://flagged[/session/{sessionId}]             - Session list (flagged filter)
 *   agentpi://state/{stateId}[/session/{sessionId}]     - Session list (state filter)
 *   agentpi://sources[/source/{sourceSlug}]          - Sources list
 *   agentpi://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   agentpi://action/{actionName}[/{id}][?params]
 *   agentpi://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *
 * Examples:
 *   agentpi://allSessions                               (all sessions view)
 *   agentpi://allSessions/session/abc123                (specific session)
 *   agentpi://settings/shortcuts                     (shortcuts page)
 *   agentpi://sources/source/github                  (github source info)
 *   agentpi://action/new-chat                        (uses active window)
 *   agentpi://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   agentpi://workspace/ws123/allSessions/session/abc123   (targets specific workspace)
 */
...
const DEEPLINK_PROTOCOL = `${process.env.CRAFT_DEEPLINK_SCHEME || APP_DEEPLINK_SCHEME}:`
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    // e.g., agentpi://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., agentpi://allSessions/chat/abc → hostname='allSessions', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // agentpi://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // agentpi://allSessions/..., agentpi://settings/..., etc. (compound routes)
    ...
    // agentpi://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // agentpi://action/... (no workspace - uses active window)
    if (host === 'action') {
