...
- **项目物理隔离**：会话开始后锁定工作目录，项目记忆固定写入该目录下 `.agent-pi/brain`；gbrain 作为项目级高级记忆后端按工作目录命名空间隔离，跨项目复用必须通过独立的公司/行业知识源显式引用。
...
后续 V1.1-V1.4 的开发方向不再拆成多个零散小版本，而是收敛为一个 **文档工作台质量与项目隔离包**：Goal Loop 深度审稿、正式输出优先预览、Markdown 渲染态编辑、PDF/DOCX 导出、工作目录强绑定、Project Memory Lite 物理隔离和可控高级知识源治理会作为同一组能力迭代。
...
| gbrain 高级模式 | Workspace 高级设置支持启用项目级 gbrain 后端；本地 PGLite 通过 `gbrain serve` stdio source 接入，远程 MCP 通过 HTTP source + bearer 认证接入；会话绑定工作目录后自动注入项目 namespace，只允许同一 workingDirectory 的会话复用。 |
