...
/**
 * Get the formal output directory for a session.
 *
 * When the user selected a working directory, formal deliverables should live
 * inside that working directory instead of the app's internal session cache.
 * The per-session subfolder keeps concurrent conversations separated.
 */
export function getSessionOutputPathFromSessionPath(sessionPath: string, workingDirectory?: string): string {
  const sessionId = basename(sessionPath);
  if (workingDirectory && !pathStartsWith(workingDirectory, sessionPath)) {
    return join(workingDirectory, FORMAL_OUTPUTS_DIR_NAME, sessionId);
  }
  return join(sessionPath, 'outputs');
}
...
export async function createSession(
  ...
  options?: {
    ...
    goalState?: SessionConfig['goalState'];
    hidden?: boolean;
