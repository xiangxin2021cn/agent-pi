...
// System prompt for Agent π context
  ...
  switch (code) {
    ...
    case 'CAPABILITY_UNAVAILABLE':
      return 'No connected desktop client supports browser tools, or no client is currently connected. ' +
        'Ask the user to open this workspace from the Agent π desktop app.';
...
export class PiAgent extends BaseAgent {
  protected backendName = 'Agent π Backend';
    ...
    const refreshPromise = (async () => {
      ...
      try {
        if (piAuthProvider === 'github-copilot') {
          ...
          const { refreshGitHubCopilotOAuth } = await import('./github-copilot-oauth.ts');
          const newCreds = await refreshGitHubCopilotOAuth(stored.refreshToken);
  ...
  private handleSubprocessEvent(event: Record<string, unknown>): void {
    ...
    // Feed them through PiEventAdapter to convert to Agent πEvents.
