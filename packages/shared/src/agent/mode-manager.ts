...
export function getPathHint(targetPath: string, plansFolderPath: string, dataFolderPath?: string): string | null {
  ...
  const isAppConfigPath = normalizedTarget.includes('/.agent-pi/') || normalizedTarget.includes('/.craft-agent/');
  const isDefaultWorkspacePath = normalizedTarget.includes('/.agent-pi/workspaces/') || normalizedTarget.includes('/.craft-agent/workspaces/');
  ...
  if (isDefaultWorkspacePath && !normalizedTarget.includes('/sessions/')) {
    return 'Hint: Write to the session plans or data folder, not the workspace root.';
  }

  // Case: Writing outside the app config directory entirely
  if (!isAppConfigPath) {
