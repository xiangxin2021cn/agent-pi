 ...
 * - Per-source: ~/.agent-pi/workspaces/{slug}/sources/{sourceSlug}/permissions.json
 *
 ...
 * Default permissions are stored at ~/.agent-pi/permissions/
 * Reads env vars dynamically so tests can override the config dir.
 */
...
/**
 * Load default permissions from ~/.agent-pi/permissions/default.json
...
class PermissionsConfigCache {
  ...
  // App-level default permissions (loaded from ~/.agent-pi/permissions/default.json)
   ...
   * These come from ~/.agent-pi/permissions/default.json
   */
