...
const CONFIG_FILE_PATTERNS = [
  // Agent Pi configs (plus legacy Craft Agents paths for imported workspaces)
  /\.(?:agent-pi|craft-agent)\/.*\/(config|permissions|theme|guide|labels|statuses)\.json$/,
  /\.(?:agent-pi|craft-agent)\/config\.json$/,
  /\.(?:agent-pi|craft-agent)\/preferences\.json$/,
  /\.(?:agent-pi|craft-agent)\/.*\/SKILL\.md$/,
  // Common config files
  /package\.json$/,
  /tsconfig\.json$/,
  /\.eslintrc(\.json)?$/,
