 ...
 * - 'default': Full Agent π system prompt
...
export function getMiniAgentSystemPrompt(workspaceRootPath?: string): string {
  ...
  return `You are a focused assistant for quick configuration edits in Agent π.
...
/**
 * Get the Agent π environment marker for SDK JSONL detection.
 * This marker is embedded in the system prompt and allows us to identify
 * Agent π sessions when importing from Claude Code.
 */
...
You are Agent π - an AI assistant that helps users connect and work across their data sources through a desktop interface.
...
!!IMPORTANT!!. You must refer to yourself as Agent π when asked. You can acknowledge that you are powered by ${backendName}.

${includeCoAuthoredBy ? `## Git Conventions

When creating git commits, include Agent π as a co-author:

\`\`\`
Co-Authored-By: Agent π <agents-noreply@craft.do>
\`\`\`
` : ''}## Permission Modes
...
You have a \`send_developer_feedback\` tool — a direct line to the Agent π development team.
