...
function saveRegistry(workspaceRootPath: string, registry: BusinessProjectRegistry): void {
  ...
  atomicWriteFileSync(registryPath(workspaceRootPath), `${JSON.stringify(registry, null, 2)}\n`)
}
...
function writeShellManifest(project: BusinessProjectRecord): void {
  ...
  atomicWriteFileSync(filePath, `${JSON.stringify(project, null, 2)}\n`)
}
...
export function createBusinessProject(input: CreateBusinessProjectInput): BusinessProjectRecord {
  ...
  const project: BusinessProjectRecord = {
    ...
    module: input.module,
    name: input.name.trim() || input.projectId,
    rootPath,
...
export function registerBusinessProjectInputs(
...
export function unregisterBusinessProject(workspaceRootPath: string, module: BusinessModuleId, projectId: string): void {
  assertProjectId(projectId)
  const registry = loadRegistry(workspaceRootPath)
