...
const FALLBACK_CONFIG_DEFAULTS: ConfigDefaults = {
  version: '1.0',
  description: 'Default configuration values for Agent π',
  defaults: {
...
/**
 * Load config defaults from ~/.agent-pi/config-defaults.json
...
/**
 * Snapshot an existing config.json into a dated file and keep the newest few.
 * Best-effort: a backup failure must not block app startup.
 */
export function backupConfigFile(): void {
  try {
    ...
    // Preserve the first good snapshot of the day; later startup attempts should
    // not overwrite it after a bad write or reset.
...
export function ensureConfigDir(): void {
  ...
  backupConfigFile();
  // Initialize bundled docs (creates ~/.agent-pi/docs/ with sources.md, agents.md, permissions.md)
  initializeDocs();
...
/**
 * Get the path to the app-level theme override file (~/.agent-pi/theme.json).
 */
 ...
 * Preset themes are stored at ~/.agent-pi/themes/
 */
...
/**
 * Returns the path to the tool-icons directory: ~/.agent-pi/tool-icons/
 */
