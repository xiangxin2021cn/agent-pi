  ...
  try {
    ...
    return {
      ...
      errors: [{
        ...
        severity: 'error',
        suggestion: 'See ~/.agent-pi/docs/skills.md for SKILL.md format reference',
      }],
      warnings: [],
    };
  }
...
/**
 * Zod schema for app-level theme override files (~/.agent-pi/theme.json).
        ...
        if (tool.icon) {
          ...
          if (!existsSync(iconPath)) {
            result.warnings.push({
              ...
              severity: 'warning',
              suggestion: `Place '${tool.icon}' in ~/.agent-pi/tool-icons/`,
            });
          }
        }
      }
 ...
 * Checks paths relative to CONFIG_DIR (~/.agent-pi/).
 * Returns null if the path is not a recognized app-level config file.
 *
 * Matches patterns:
 * - ~/.agent-pi/tool-icons/tool-icons.json → tool icon mappings
 */
