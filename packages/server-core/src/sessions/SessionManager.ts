...
type ResolvedSessionConnection = NonNullable<ReturnType<typeof resolveSessionConnection>>

function connectionAllowsModel(connection: ResolvedSessionConnection | null | undefined, model: string | null): boolean {
  if (!model || !connection) return true
  const modelIds = connection.models
    ?.map(entry => typeof entry === 'string' ? entry : entry.id)
    .filter((id): id is string => Boolean(id))

  // Older/custom connections may not have a synced model catalog. In that
  // case we cannot safely validate membership, so keep the legacy behavior.
  if (!modelIds || modelIds.length === 0) return true

  return modelIds.includes(model) || connection.defaultModel === model
}
...
async function getBrowserToolIconDataUrl(): Promise<string | undefined> {
  ...
  try {
    const iconCandidates = [
      ...
      // Dev fallback (before sync to ~/.agent-pi/tool-icons)
...
async function resolveToolDisplayMeta(
  ...
  // and resolves their brand icon from ~/.agent-pi/tool-icons/
...
function buildGoalReviewPrompt(input: GoalReviewInput): string {
  ...
  return [
    ...
    '- Do not pass outputs that broaden beyond selected sources, named chapters/files/folders, requested output format, or response language.',
...
export class SessionManager implements ISessionManager {
  ...
  private loadAllSourcesForSession(managed: ManagedSession): LoadedSource[] {
    return loadAllSources(managed.workspace.rootPath)
  }
  ...
  private applyExternalSessionMetadata(managed: ManagedSession, header: SessionHeader): boolean {
    ...
    if (managed.parentSessionId !== header.parentSessionId) {
      managed.parentSessionId = header.parentSessionId
      structuralChanged = true
      changed = true
    }

    if (managed.parentSessionKind !== header.parentSessionKind) {
      managed.parentSessionKind = header.parentSessionKind
      structuralChanged = true
      changed = true
    }

    if (managed.branchFromMessageId !== header.branchFromMessageId) {
      managed.branchFromMessageId = header.branchFromMessageId
      structuralChanged = true
      changed = true
    }

    if (structuralChanged) {
      this.sendEvent({ type: 'session_metadata_changed', sessionId }, managed.workspace.id)
    }
  ...
  private loadSessionsFromDisk(): void {
    try {
      ...
      for (const workspace of workspaces) {
        ...
        this.inferLegacyParentSessionLinks(workspaceRootPath, sessionMetadata)
  ...
  private inferLegacyParentSessionLinks(workspaceRootPath: string, metadata: SessionMetadata[]): void {
    const sessionsById = new Map(metadata.map(meta => [meta.id, meta]))
    const legacyBranches = metadata.filter(meta => meta.branchFromMessageId && !meta.parentSessionId)
    if (legacyBranches.length === 0) return

    let inferredCount = 0
    const loadedSessions = new Map<string, StoredSession | null>()
    const loadForInference = (sessionId: string): StoredSession | null => {
      if (!loadedSessions.has(sessionId)) {
        loadedSessions.set(sessionId, loadStoredSession(workspaceRootPath, sessionId))
      }
      return loadedSessions.get(sessionId) ?? null
    }

    for (const meta of legacyBranches) {
      let inferredParentId: string | undefined

      if (meta.branchFromSessionPath) {
        const candidateId = basename(meta.branchFromSessionPath)
        if (candidateId && candidateId !== meta.id && sessionsById.has(candidateId)) {
          inferredParentId = candidateId
        }
      }

      if (!inferredParentId && meta.branchFromMessageId) {
        const candidates: string[] = []
        const childCreatedAt = meta.createdAt || Number.MAX_SAFE_INTEGER

        for (const candidate of metadata) {
          if (candidate.id === meta.id) continue
          if ((candidate.createdAt || 0) > childCreatedAt) continue

          const stored = loadForInference(candidate.id)
          if (stored?.messages.some(message => message.id === meta.branchFromMessageId)) {
        ...
        if (candidates.length === 1) {
          inferredParentId = candidates[0]
        }
      }

      if (inferredParentId) {
        meta.parentSessionId = inferredParentId
        meta.parentSessionKind = 'branch'
        inferredCount++
      }
    }

    if (inferredCount > 0) {
      sessionLog.info(`Inferred parent session links for ${inferredCount} legacy branch session(s) in ${workspaceRootPath}`)
    }
  }
  ...
  private async getOrCreateAgent(managed: ManagedSession): Promise<AgentInstance> {
    ...
    if (!managed.agent) {
      ...
      managed.agent.onSourceActivationRequest = async (sourceSlug: string): Promise<boolean> => {
        ...
        const sources = this.getSourcesBySlugsForSession(managed, [sourceSlug])
        if (sources.length === 0) {
        ...
        const allEnabledSources = this.getSourcesBySlugsForSession(managed, managed.enabledSourceSlugs || [])
        // Pass session path so large API responses can be saved to session folder
        const sessionPath = getSessionStoragePath(workspaceRootPath, managed.id)
  ...
  async setSessionSources(sessionId: string, sourceSlugs: string[]): Promise<void> {
    ...
    if (managed.agent) {
      const sources = this.getSourcesBySlugsForSession(managed, sourceSlugs)
      // Pass session path so large API responses can be saved to session folder
  ...
  async updateSessionModel(sessionId: string, workspaceId: string, model: string | null, connection?: string): Promise<void> {
    ...
    if (managed) {
      ...
      const currentConnectionSlug = currentConnection?.slug ?? managed.llmConnection
      const isChangingConnection = Boolean(connection && connection !== currentConnectionSlug)
      ...
      if (!connectionAllowsModel(targetConnection, model)) {
  ...
  async sendMessage(
    ...
    if (options?.skillSlugs?.length) {
      try {
        ...
        if (requiredSources.size > 0) {
          ...
          const loadedSources = this.getSourcesBySlugsForSession(managed, candidateSlugs)
    ...
    const sources: LoadedSource[] = hasSources
      ? this.getSourcesBySlugsForSession(managed, enabledSlugs)
      : []
