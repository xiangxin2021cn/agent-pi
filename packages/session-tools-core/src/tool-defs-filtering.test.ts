...
describe('session tool filtering helpers', () => {
  ...
  it('safe-mode helper sets classify expected tools', () => {
    ...
    expect(allowed.has('spawn_session')).toBe(true);
    expect(allowed.has('send_agent_message')).toBe(true);
  ...
  it('safe-mode helpers support MCP prefixing', () => {
    ...
    expect(allowedPrefixed.has('mcp__session__spawn_session')).toBe(true);
    expect(allowedPrefixed.has('mcp__session__send_agent_message')).toBe(true);
